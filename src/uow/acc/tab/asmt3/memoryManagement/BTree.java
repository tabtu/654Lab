package uow.acc.tab.asmt3.memoryManagement;
...
public class BTree<Key extends Comparable<Key>, Value>  {
    ...
    public void traversal(String file) {
        out = new Out(path + file);
        traversal(root, HT);
    }
    private void traversal(Node x, int ht) {
        Entry[] children = x.children;

        // external node
        if (ht == 0) {
            for (int j = 0; j < x.m; j++) {
                out.println(children[j].key + "/" + children[j].value);
            }
        }
        ...
        // internal node
            ...
            for (int j = 0; j < x.m; j++) {
                traversal(children[j].next, ht - 1);
            }
        }
    }
